	...
	case accessibilityRange(ClosedRange<Double>)
	 ...
	 var queryItems: [String: String] {
		switch self {
			...
			case .type(let activityType): return ["type": activityType.rawValue]
			case .participants(let people): return ["participants": "\(people)"]
			case .price(let price): return ["price": "\(price)"]
			...
			case .accessibility(let value): return ["accessibility": "\(value)"]
